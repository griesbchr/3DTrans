...
class GACEDataset(Dataset):
    ...
    def __init__(self, cfg, model, dataloader, logger, train):
        ...
        #self.logger.gace_info(msg)
        self.generate_data(model, dataloader)
...
#
    ...
    def generate_data(self, model, dataloader):
        ...
        est_shape = (len(dataloader.dataset) * nr_max_det_per_sample, len(self.ip_dict))
        ip_data = np.zeros(est_shape, dtype=np.float32)
        
        est_shape = (len(dataloader.dataset) * nr_max_det_per_sample, len(self.target_dict))
        target_data = np.zeros(est_shape, dtype=np.float32)

        est_shape = (len(dataloader.dataset) * nr_max_det_per_sample, self.max_nr_neighbors)
