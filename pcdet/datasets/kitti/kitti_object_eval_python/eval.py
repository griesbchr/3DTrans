...
def get_custom_eval_result(gt_annos, dt_annos, current_classes, PR_detail_dict=None):
    '''
    kitti style evaluation for a single level of difficulty, only 3d bbox and two different overlap settings
    '''
    
                            # classes -->
    overlap_0_7 = np.array([[0.7, 0.5, 0.5],       #easy
                            [0.7, 0.5, 0.5],       #moderate
                            [0.7, 0.5, 0.5]])      #hard
    overlap_0_5 = np.array([[0.5, 0.25, 0.25],
                            [0.5, 0.25, 0.25],
                            [0.5, 0.25, 0.25]])
    min_overlaps_per_class = np.stack([overlap_0_7, overlap_0_5], axis=0)  # [2, 3, 3]
    class_to_name = {
        0: 'Car',
        1: 'Pedestrian',
        2: 'Cyclist'
    ...
    for curcls in current_classes:
        if isinstance(curcls, str):
            ...
            current_classes_int.append(curcls)
    current_classes = current_classes_int
    min_overlaps = min_overlaps_per_class[:, :, current_classes]
    result = print_str('\n')
    ...
    ret = eval_class(gt_annos, dt_annos, current_classes, difficultys, 2,
                     min_overlaps)
    # mAP3d: [num_class, num_diff, num_minoverlap]
    mAP3d = get_mAP(ret["precision"])
    mAP3d_R40 = get_mAP_R40(ret["precision"])
    if PR_detail_dict is not None:
        PR_detail_dict['3d'] = ret['precision']
    ...
    for j, curcls in enumerate(current_classes):
        ...
        #result += print_str(
        #    (f"{class_to_name[curcls]} "
        #        "AP_R11@{:.2f}, {:.2f}:".format(*min_overlaps[:, difficultys, j].flatten())))
        #result += print_str("3d     AP:{:.2f}, {:.2f} ".format(*mAP3d[j, 0, :]))

        result += print_str(
            (f"{class_to_name[curcls]} "
                "AP_R40@{:.2f}, {:.2f}:".format(*min_overlaps[:, difficultys, j].flatten())))
        result += print_str("3d     AP:{:.2f}, {:.2f} ".format(*mAP3d_R40[j, 0, :]))

        result += print_str("")

        if j == 0:
            ret_dict['%s_3d/moderate' % class_to_name[curcls]] = mAP3d[j, 0, 0]
            ret_dict['%s_3d/moderate_R40' % class_to_name[curcls]] = mAP3d_R40[j, 0, 0]
