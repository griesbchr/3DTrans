...
class DataBaseSampler(object):
    ...
    def add_sampled_boxes_to_scene(self, data_dict, sampled_gt_boxes, total_valid_sampled_dict):
        ...
        for idx, info in enumerate(total_valid_sampled_dict):
            if self.use_shared_memory:
                ...
                if self.oss_flag:
                    ...
                    assert obj_points.shape[0] == info['num_points_in_gt'], f"{file_path} {obj_points.shape[0]} != {info['num_points_in_gt']}"
        ...
        if 'truncated' in data_dict:
            data_dict['truncated'] = np.concatenate([data_dict['truncated'], np.zeros_like(sampled_gt_names, dtype=bool)], axis=0)
